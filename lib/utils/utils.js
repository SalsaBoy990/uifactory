...
module.exports = {
  ...
  // save newsdata
  saveNewsDataForIndexPage: saveNewsDataForIndexPage,
...
// save postdata for the index page
function savePostDataForIndexPage (fileData, dateFormatted, postData, results, dir) {
  'use strict'
  ...
  if (dir === 'posts') {
    // Store the paths to the files, in descending order by date
    ...
    throw new Error('Wrong directory name supplied! Please check the 5th argument for savePostDataForIndexPage().')
  }
}

// save postdata for the index page
function saveNewsDataForIndexPage (dateFormatted, postData, postContents, results, dir) {
  'use strict'
  // Store the paths to the files, in descending order by date
  // Here, the md files are in ascending order (oldest first),
  // so we need to be reverse the order for the index page
